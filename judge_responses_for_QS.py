...
import re
import numpy as np
from utils import existing_model_paths_QS
...
def load_model(model_name,tensor_parallel_size, enforce_eager=True):
    model_info = existing_model_paths_QS.get(model_name)
    ...
    if os.path.exists(model_info):
        ...
        vllm_model = LLM(model=model_info, tokenizer=tokenizer_str, gpu_memory_utilization=0.7, tensor_parallel_size=tensor_parallel_size, enforce_eager=True)
...
def save_judgement(judge_name, data, dimension, split=0):
    ...
    if split != 0:
        path = f"judgements_{dimension}/{judge_name}/judgements_{split}.jsonl"
    else:
        path = f"judgements_{dimension}/{judge_name}/judgements.jsonl"
...
def run_judging_trials(path="math_questions.jsonl", model_name="command-r-v01", tensor_parallel_size=1, dimension="math_algebra", split=0):
    print(path, model_name, dimension)
    global total_completion_cost, total_prompt_cost
    model_names = list(existing_model_paths_QS.keys())

    start_time = time.time()
    initial_question_ids = range(0, 500) #range(0, 778)
...
if __name__ == '__main__':
    ...
    fire.Fire(run_judging_trials(path=args.path, model_name=args.model_name, tensor_parallel_size=args.tensor_parallel_size, dimension=args.dimension, split=args.split))
